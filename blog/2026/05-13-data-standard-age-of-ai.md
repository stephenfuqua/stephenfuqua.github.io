---
title: "The Ed-Fi Data Standard in the Age of AI: Benefits, Limitations, and a Path Forward"
keywords:
- ed-fi
- AI
- data-and-analytics
...
Originally written for the [Ed-Fi Alliance technology blog](https://docs.ed-fi.org).
...
The Ed-Fi [Analytics Middle Tier](https://docs.ed-fi.org/getting-started/edfi-exchange/technology/analytics-middle-tier/amt-overview) addressed this gap for direct database consumers by providing [row-level security views](https://docs.ed-fi.org/getting-started/edfi-exchange/technology/analytics-middle-tier/user-guide/patterns-for-row-level-user-security) that map users to authorization scopes — district, school, section, or individual student. These views support both dynamic authorization (time-sensitive, based on current enrollment) and static authorization (broader access without temporal restrictions). But these views are SQL constructs tied to the ODS database; they are not accessible through the API, and they are not designed for the kinds of downstream data stores where AI and analytics workloads should run.
...
The Analytics Middle Tier's [row-level security patterns](https://docs.ed-fi.org/getting-started/edfi-exchange/technology/analytics-middle-tier/user-guide/patterns-for-row-level-user-security) provide a proven model for user-level authorization in educational data. The semantic layer must implement equivalent controls:
...
* What tooling would be needed to generate and maintain the graph schema as the Data Standard evolves? Depending on complexity, this could be an [additional output](https://docs.ed-fi.org/reference/metaed/ide-user-guide/using-the-generated-artifacts) from the MetaEd build process.
...
_This white paper was developed by the Ed-Fi Alliance. It was substantially written with the help of an AI assistant based on a detailed outline and additional conversational elicitation with Stephen Fuqua, Head of Product Development._
